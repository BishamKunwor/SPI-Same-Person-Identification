...
export default function ClientBankAccForm({ prev = () => { }, next = () => { }, calledFrom = "edit" }: ClientBankAccArgs) {
    ...
    return <>
        <Form onFinish={(data) => { console.log(data); next(); }}>
            ...
            <FormLabel>Your provided bank details will be used to trasfer returns of your investment at end of stated period of Portfolio Management Service.</FormLabel>
            ...
            <Row gutter={[48, 0]}>
                <FormItemWrapper label="Account Number*" colProps={{ span: 8 }} >
                    <Input disabled={isEditable} size="large" placeholder="Enter bank account number" />
                </FormItemWrapper>
                <FormItemWrapper label="Bank*" colProps={{ span: 8 }} >
                    <Select disabled={isEditable} size="large" placeholder="Select from dropdown" />
                </FormItemWrapper>
                <FormItemWrapper label="Bank Branch Name*" colProps={{ span: 8 }} >
                    <Select disabled={isEditable} size="large" placeholder="Select from dropdown" />
                </FormItemWrapper>
            </Row>
            {
